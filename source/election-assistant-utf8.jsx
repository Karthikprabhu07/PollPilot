...
const LANGUAGES = [
  ...
  { code: "ig", label:const TRANSLATIONS = {
  en: {
...
�े", noHistory: "अजून संभाषण नाही",
  ...
  ml: {
    newChat: "പുതിയ ചാറ്റ്", search: "തിരയുക", selectLang: "ഭാഷ തിരഞ്ഞെടുക്കുക", signIn: "സൈൻ ഇൻ ചെയ്യുക", signOut: "സൈൻ ഔട്ട്", guestMode: "ഗസ്റ്റ് മോഡ്",
    welcomeTitle: "നിങ്ങളുടെ തിരഞ്ഞെടുപ്പ് സഹായി", welcomeSubtitle: "ഓരോ ജനാധിപത്യത്തിനും സിവിക് ഇൻ്റലിജൻസ്", selectYourLang: "നിങ്ങളുടെ ഭാഷ തിരഞ്ഞെടുക്കുക", currently: "നിലവിൽ", continueIn: "തുടരുക",
    placeholder: "PollPilot-ലേക്ക് സന്ദേശം അയക്കുക...", footer: "PollPilot തെറ്റുകൾ വരുത്തിയേക്കാം. പ്രധാന വിവരങ്ങൾ പരിശോധിക്കുക.",
    historyToday: "ഇന്ന്", historyYesterday: "ഇന്നലെ", historyOlder: "പഴയത്", historyPinned: "പിൻ ചെയ്തു", noHistory: "സംഭാഷണങ്ങളൊന്നുമില്ല",
    askVoter: "വോട്ടർ രജിസ്ട്രേഷനെക്കുറിച്ച് ചോദിക്കുക →", rename: "പേര് മാറ്റുക", delete: "ഡിലീറ്റ് ചെയ്യുക", pin: "ചാറ്റ് പിൻ ചെയ്യുക", unpin: "ചാറ്റ് അൺപിൻ ചെയ്യുക", retry: "വീണ്ടും ശ്രമിക്കുക",
    prompts: ["ഞാൻ എങ്ങനെ വോട്ട് ചെയ്യാൻ രജിസ്റ്റർ ചെയ്യും?", "വോട്ടെണ്ണൽ സമയത്ത് എന്താണ് സംഭവിക്കുന്നത്?", "അടുത്ത തിരഞ്ഞെടുപ്പുകൾ എപ്പോഴാണ്?", "നിയമങ്ങളും യോഗ്യതയും"]
  },
  bn: {
    newChat: "নতুন চ্যাট", search: "খুঁজুন", selectLang: "ভাষা নির্বাচন করুন", signIn: "সাইন ইন করুন", signOut: "সাইন আউট", guestMode: "গেস্ট মোড",
    welcomeTitle: "আপনার নির্বাচন গাইড", welcomeSubtitle: "প্রতিটি গণতন্ত্রের জন্য নাগরিক বুদ্ধিমত্তা", selectYourLang: "আপনার ভাষা নির্বাচন করুন", currently: "বর্তমানে", continueIn: "চালিয়ে যান",
...
function ElectionAssistant() {
  ...
  const handleSend = async (text = input, isRetry = false) => {
    ...
    if (!isRetry) { 
      ...
      newMessages = [...messages.slice(0, -1), userMsg];
      setMessages(newMessages);
    }
    ...
    if (!currentSessionId) {
      ...
      const title = userMsg.content.slice(0, 35) + (userMsg.content.length > 35 ? "…" : "");
    ...
    try {
      const rawHistory = isRetry ? messages.slice(0, -1) : [...messages, userMsg];
      const cleanHistory = [];
      ...
      setMessages(prev => {
        ...
        const updated = [...prev, { role: "assistant", content: errorMessage, isError: true, lastQuery: trimmed }];
        setHistorySessions(hist => hist.map(s => s.id === currentSessionId ? { ...s, messages: updated } : s));
        return updated;
      });
    }
      ...
      <main className="pp-main">
        ...
        <div className="chat-area">
          {!hasMessages ? (
            ...
            messages.map((m, i) => (
              <div key={i} className={`msg-wrap ${m.role !== 'user' ? 'msg-bot-wrap' : 'msg-user-wrap'}`}>
